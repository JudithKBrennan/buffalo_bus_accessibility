...
from utils import getDirectory, getResults, checkPreference, getExperimentOD, \
    getAPIKey
...
def isCloseTo(i: list, j:list, k:list):
    '''
    Returns true iff k is on or next to (with some tolerance) the line connecting i and j
    '''
    tolerance = 1e-8
...
def getCommuteShape(trip_shape, bus_start, bus_end):
    ...
    return shape #return as a np.array
...
def viewRoute(result: pd.DataFrame, origin: int, destination: int,\
               preference: str, directory: str):
    '''
...
if __name__ == '__main__':
    ...
    result = route_preferences(all_routes,args.time_of_day,args.origins_id,\
                               args.destination_id,preference)
    ...
    viewRoute(result, origin, destination, directory)
